from transformers import AutoTokenizer, AutoModelForCausalLM, pipeline
import os
import json
import torch
from typing import Dict, List, Any

# Initialize Hugging Face model
def get_model():
    """Initialize and return the Hugging Face model and tokenizer."""
    model_name = "microsoft/DialoGPT-medium"  # Lightweight conversational model
    
    try:
        tokenizer = AutoTokenizer.from_pretrained(model_name)
        model = AutoModelForCausalLM.from_pretrained(model_name)
        
        # Add padding token if not present
        if tokenizer.pad_token is None:
            tokenizer.pad_token = tokenizer.eos_token
            
        return pipeline("text-generation", model=model, tokenizer=tokenizer, max_new_tokens=1000)
    except Exception as e:
        print(f"Error loading model: {e}")
        return None

# Global model instance
model_pipeline = get_model()

SYSTEM_PROMPT = """
You are a UK bank regulatory reporting assistant specializing in PRA COREP reporting.
Your task is to analyze user scenarios and generate structured regulatory output.

CRITICAL RULES:
1. Use ONLY the provided regulatory context from the retrieved documents
2. Output STRICT JSON following the exact schema provided
3. If data is missing or unclear, set amount=null and explain in data_gaps
4. All amounts should be in the currency specified (default GBP)
5. Provide specific regulatory references for each populated field
6. Do not invent values or make assumptions beyond the context provided
7. Focus on COREP Template C 01.00 - Own Funds reporting
"""

def generate_corep_output(user_query: str, retrieved_docs: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Generate structured COREP output using Hugging Face LLM based on user query and retrieved documents.
    
    Args:
        user_query (str): Natural language scenario from the user
        retrieved_docs (list): List of retrieved regulatory documents
        
    Returns:
        dict: Structured COREP output in JSON format
    """
    
    if model_pipeline is None:
        return {
            "template": "C 01.00",
            "currency": "GBP",
            "error": "Hugging Face model not available",
            "own_funds": {
                "CET1": {"ordinary_share_capital": {"amount": None}, "retained_earnings": {"amount": None}, "intangibles_deduction": {"amount": None}},
                "AT1": {"instruments": {"amount": None}},
                "Tier2": {"instruments": {"amount": None}}
            },
            "data_gaps": [{"field": "all", "issue": "Model not loaded", "suggestion": "Check model installation"}]
        }
    
    # Format retrieved context
    context = "\n\n".join([
        f"SOURCE: {d['source']}\n{d['text']}" 
        for d in retrieved_docs
    ])
    
    prompt = f"""
REGULATORY CONTEXT:
{context}

USER SCENARIO:
{user_query}

TASK: Generate structured COREP output for Template C 01.00 - Own Funds.

Return JSON in this exact format:

{{
  "template": "C 01.00",
  "currency": "GBP",
  "reporting_date": "2026-01-31",
  "own_funds": {{
    "CET1": {{
      "ordinary_share_capital": {{
        "amount": number or null,
        "corep_row": "010",
        "justification_refs": ["source1", "source2"],
        "explanation": "Brief explanation of why this amount is included"
      }},
      "retained_earnings": {{
        "amount": number or null,
        "corep_row": "020", 
        "justification_refs": ["source1", "source2"],
        "explanation": "Brief explanation of why this amount is included"
      }},
      "intangibles_deduction": {{
        "amount": number or null,
        "corep_row": "350",
        "justification_refs": ["source1", "source2"],
        "explanation": "Brief explanation of why this amount is deducted"
      }}
    }},
    "AT1": {{
      "instruments": {{
        "amount": number or null,
        "corep_row": "120",
        "justification_refs": ["source1", "source2"],
        "explanation": "Brief explanation of why this amount is included"
      }}
    }},
    "Tier2": {{
      "instruments": {{
        "amount": number or null,
        "corep_row": "200",
        "justification_refs": ["source1", "source2"],
        "explanation": "Brief explanation of why this amount is included"
      }}
    }}
  }},
  "data_gaps": [
    {{
      "field": "field_name",
      "issue": "description of missing/unclear data",
      "suggestion": "how to resolve"
    }}
  ],
  "summary": {{
    "total_cet1": number or null,
    "total_at1": number or null,
    "total_tier2": number or null,
    "total_own_funds": number or null
  }}
}}

IMPORTANT:
- Amounts should be numeric values (not strings)
- Use null for missing/unclear amounts
- Provide specific regulatory references from the context
- Include explanations for each populated field
- List any data gaps or uncertainties

JSON Response:
"""

    try:
        # Generate response using Hugging Face model
        response = model_pipeline(
            prompt,
            max_new_tokens=1500,
            temperature=0.1,
            do_sample=True,
            pad_token_id=model_pipeline.tokenizer.eos_token_id
        )
        
        result_text = response[0]['generated_text']
        
        # Extract the generated part (remove the prompt)
        if prompt in result_text:
            result_text = result_text.replace(prompt, "").strip()
        
        # Parse JSON response
        try:
            result = json.loads(result_text)
        except json.JSONDecodeError:
            # Try to extract JSON from response if it contains extra text
            start_idx = result_text.find('{')
            end_idx = result_text.rfind('}') + 1
            if start_idx != -1 and end_idx != 0:
                json_text = result_text[start_idx:end_idx]
                result = json.loads(json_text)
            else:
                # Fallback to basic structure if JSON parsing fails
                result = create_fallback_response(user_query, retrieved_docs)
        
        # Validate required fields
        if "template" not in result:
            result["template"] = "C 01.00"
        if "currency" not in result:
            result["currency"] = "GBP"
        if "data_gaps" not in result:
            result["data_gaps"] = []
        
        return result
        
    except Exception as e:
        # Return error structure
        return {
            "template": "C 01.00",
            "currency": "GBP",
            "error": f"Hugging Face model generation failed: {str(e)}",
            "own_funds": {
                "CET1": {"ordinary_share_capital": {"amount": None}, "retained_earnings": {"amount": None}, "intangibles_deduction": {"amount": None}},
                "AT1": {"instruments": {"amount": None}},
                "Tier2": {"instruments": {"amount": None}}
            },
            "data_gaps": [{"field": "all", "issue": f"Model processing error: {str(e)}", "suggestion": "Check model installation and try again"}]
        }

def create_fallback_response(user_query: str, retrieved_docs: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Create a basic fallback response when model fails to generate proper JSON."""
    
    # Simple rule-based extraction for demonstration
    response = {
        "template": "C 01.00",
        "currency": "GBP",
        "reporting_date": "2026-01-31",
        "own_funds": {
            "CET1": {
                "ordinary_share_capital": {"amount": None, "corep_row": "010", "justification_refs": [], "explanation": "Could not extract from query"},
                "retained_earnings": {"amount": None, "corep_row": "020", "justification_refs": [], "explanation": "Could not extract from query"},
                "intangibles_deduction": {"amount": None, "corep_row": "350", "justification_refs": [], "explanation": "Could not extract from query"}
            },
            "AT1": {
                "instruments": {"amount": None, "corep_row": "120", "justification_refs": [], "explanation": "Could not extract from query"}
            },
            "Tier2": {
                "instruments": {"amount": None, "corep_row": "200", "justification_refs": [], "explanation": "Could not extract from query"}
            }
        },
        "data_gaps": [
            {"field": "model_processing", "issue": "Hugging Face model could not generate structured output", "suggestion": "Manual data entry required"}
        ],
        "summary": {
            "total_cet1": None,
            "total_at1": None,
            "total_tier2": None,
            "total_own_funds": None
        }
    }
    
    return response

def test_llm_connection():
    """Test the Hugging Face model connection."""
    try:
        if model_pipeline is None:
            return "Hugging Face model not loaded"
        
        test_prompt = "Hello, can you respond with 'Model connection successful'?"
        response = model_pipeline(
            test_prompt,
            max_new_tokens=50,
            temperature=0.1,
            do_sample=True,
            pad_token_id=model_pipeline.tokenizer.eos_token_id
        )
        
        result = response[0]['generated_text']
        if test_prompt in result:
            result = result.replace(test_prompt, "").strip()
        
        return result if result else "Model connection successful"
    except Exception as e:
        return f"Connection failed: {str(e)}"

if __name__ == "__main__":
    # Test the model connection
    print("Testing Hugging Face model connection...")
    result = test_llm_connection()
    print(f"Result: {result}")
    
    # Example usage with mock data
    if "successful" in result.lower() or model_pipeline is not None:
        mock_query = "The bank has £120m ordinary share capital, £30m retained earnings, £10m AT1 instruments, and £8m intangible assets."
        mock_docs = [
            {
                "source": "PRA_Own_Funds.txt",
                "text": "CET1 capital includes ordinary share capital and retained earnings. Intangible assets must be deducted."
            }
        ]
        
        print("\nTesting COREP generation...")
        output = generate_corep_output(mock_query, mock_docs)
        print(json.dumps(output, indent=2))
